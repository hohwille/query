...
import net.sf.mmm.query.api.feature.FeatureGroupBy;
import net.sf.mmm.query.api.feature.FeatureLet;
import net.sf.mmm.query.api.feature.FeatureLimit;
import net.sf.mmm.query.api.feature.FeatureOrderBy;
import net.sf.mmm.query.api.feature.FeaturePaging;
import net.sf.mmm.query.api.feature.FeatureSet;
import net.sf.mmm.query.api.feature.FeatureValues;
...
import net.sf.mmm.query.api.statement.Statement;
import net.sf.mmm.query.api.variable.Variable;
import net.sf.mmm.query.base.argument.Args;
...
import net.sf.mmm.query.base.statement.DefaultSqlDialect;
...
public abstract class AbstractStatementTest<S extends Statement<QueryTestPropertyBean, S>> extends Assertions {
  ...
  private QueryTestPropertyBean prototype = BeanFactoryImpl.getInstance().createPrototype(QueryTestPropertyBean.class);
  ...
  public void testSqlOptimization() {
    ...
    String sql = getSqlStart() + quote(statement, QueryTestPropertyBean.class.getSimpleName()) + statement.getDialect().as(getAlias(), false);
  ...
  protected String checkLet(S statement, String sql, List<Object> variables) {
    ...
    String sqlLet = sql + " LET " + statement.getDialect().variable(variable) + " = " + property(statement, "Parent.Parent");
  ...
  protected String checkValues(S statement, String sql, List<Object> variables) {
    ...
    // add assignment to SET clause
    boolean friend = true;
    String sqlValues = sql + " (" + property(statement, "Friend") + ") VALUES (" + variable(statement, variables, friend) + ")";
    ...
    sqlValues = sql + " (" + property(statement, "Friend") + ", " + property(statement, "Orientaton") + ") VALUES (" + variable(statement, variables, friend)
  ...
  protected String checkWhere(S statement, String sql, List<Object> variables) {
    ...
    Integer min = 16;
    Integer max = 21;
    String condAge = property(statement, "Age") + " BETWEEN " + variable(statement, variables, min) + " AND " + variable(statement, variables, max);
    ...
    String pattern = "%\\%";
    char escape = '\\';
    String condName = property(statement, "LastName") + " LIKE " + variable(statement, variables, pattern) + " ESCAPE '\\'";
    ...
    Orientation orientation = Orientation.HORIZONTAL;
    sqlWhere = sqlWhere + condFirstName + " AND " + condAge + " AND " + condName + " AND (" + property(statement, "Friend") + " = FALSE OR "
        + property(statement, "Orientation") + " = " + variable(statement, variables, orientation) + ")";
    where.where(this.alias.to(this.prototype.Friend()).isFalse().or(this.alias.to(this.prototype.Orientation()).eq(orientation)));
