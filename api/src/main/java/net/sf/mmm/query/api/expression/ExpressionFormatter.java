...
public interface ExpressionFormatter {
  ...
  /**
   * @return the {@link StringBuilder} where to {@link StringBuilder#append(CharSequence) append} the
   *         {@link Expression} to {@link Expression#format(ExpressionFormatter, Bracketing) format}.
   */
  ...
  /**
   * @param object the {@link Object} to {@link StringBuilder#append(CharSequence) append} to the
   *        {@link #getBuffer() buffer}.
   */
  ...
  /**
   * @return {@code true} if {@link Conjunction#isNegation() negative} {@link Conjunction}s should be resolved
   *         via negation (e.g. {@code !(a OR b)} instead of {@code a NOR b}), {@code false} otherwise.
   */
