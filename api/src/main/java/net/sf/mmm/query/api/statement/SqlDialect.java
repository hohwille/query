...
package net.sf.mmm.query.api.statement;
...
public interface SqlDialect {
   ...
   * @param alias the {@link EntityAlias}.
   * @param space {@code true} if a space should be appended at the end of the alias, {@code false} otherwise.
   * @return the {@link EntityAlias#getName()}. In case {@link EntityAlias#getName()} is {@code null}
   *         according to the dialect either the empty string or the {@link EntityAlias#getSource() source} is
   *         used as fallback.
   */
  ...
  /**
   * @see net.sf.mmm.query.api.feature.FeatureUpsert
  ...
  /**
   * @return the {@code ESCAPE} keyword to supply an escape character for {@link #like()} expressions. Will be
   *         the empty {@link String} if NOT support via this {@link SqlDialect} (maybe only via query
   *         metadata).
   */
   ...
   * @return the {@code ESCAPE} clause to supply an escape character for {@link #like()} expressions. Will be
   *         the empty {@link String} if NOT support via this {@link SqlDialect} (maybe only via query
   *         metadata).
   */
  ...
  /**
   * @return the {@code LIMIT} keyword to supply the maximum number of results. Will be the empty
   *         {@link String} if NOT supported via this {@link SqlDialect} (maybe only via query metadata).
   */
  ...
  /**
   * @return the {@code OFFSET} (or SKIP) keyword to supply an offset. Will be the empty {@link String} if NOT
   *         support via SQL (maybe only via query metadata).
   */
  ...
  /**
   * @return the optional quote used to escape references such as selection source, paths, etc. to prevent
   *         keyword collisions. Will be the empty {@link String} for no quoting of references.
   */
  ...
  /**
   * @return the SQL separator for {@link net.sf.mmm.query.api.path.Path paths} in {@link #groupBy()
   *         GROUP BY} clause, {@link #orderBy() ORDER BY} expressions, etc. or {@link EntityAlias aliases} in
   *         {@link #from() FROM}, {@link #join() JOIN}, etc.
   */
