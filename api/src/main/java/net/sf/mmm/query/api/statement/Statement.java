...
import net.sf.mmm.query.api.Command;
...
public abstract interface Statement<E, SELF extends Statement<E, SELF>> extends Command {
  ...
  /**
   * @return the {@link EntityAlias} to work on (select from, insert into, etc.). For regular
   *         {@link Statement}s this will be {@link EntityAlias}{@code <E>} but for special queries such as
  ...
  /**
   * @return an {@link java.util.Collections#unmodifiableList(List) unmodifiable} {@link List} with the bind
   *         variables.
   */
  List<Object> getParameters();

  /**
   * @return the current SQL {@link String} of this {@link Statement}. May only be an SQL fragment as the
   *         final operation such as {@link SelectStatement#fetch()} will complete the statement. Has to be
   *         rebuild after every modification to the {@link Statement}.
   */
  @Override
  String getSql();
