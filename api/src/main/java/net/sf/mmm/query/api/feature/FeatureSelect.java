...
/**
 * {@link StatementFactoryFeature} for a {@link net.sf.mmm.query.api.statement.StatementFactory}
 * supporting a {@link SelectStatement}.
 *
 * @author hohwille
 * @since 8.5.0
 */
public abstract interface FeatureSelect extends StatementFactoryFeature {
  ...
  /**
   * Creates {@link SelectStatement} for a projection to a transfer-object having an according constructor. In
   * JPA this is called a constructor query (
   * {@code SELECT NEW package.Classname(arg1, arg2, ...) FROM alias.source [AS alias.name] ...}). If not
   * natively supported this is implemented as a projection based on a
   * {@link #selectFrom(EntityAlias, PropertyPath...) tuple selection}.
   *
   * @param <E> the generic type of the transfer-object to select.
   * @param alias the {@link EntityAlias} to select from.
   * @param toClass the transfer-object to use. Will be instantiated via the
   *        {@link java.lang.reflect.Constructor} matching to the {@link PropertyPath#getValue() value} types
   *        of the {@code constructorArgs}.
